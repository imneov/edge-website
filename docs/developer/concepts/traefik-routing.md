...
每创建一个 vCluster 子集群，Controller 就为其创建对应的 Prometheus IngressRoute。目标服务有两种形态（随版本演进）：

**新版（直接写主集群 Prometheus）**：

```yaml
# 示例：v-ke-001、v-oy-101（新版）
spec:
  routes:
    - match: Host(`v-ke-001.rise.io`)
      services:
        - name: prometheus-operated
          namespace: observability-system   # 直接写主集群
          port: 9090
```

**旧版（通过 vCluster Syncer 映射的服务）**：

```yaml
# 示例：v-ke-101、v-oy-001 等（旧版）
spec:
  routes:
    - match: Host(`v-ke-101.rise.io`)
      services:
        # vCluster synced service：prometheus-operated-x-observability-system-x-{vclusterRelease}
        - name: prometheus-operated-x-observability-system-x-vcluster-vcluster-v-ke-101
          namespace: vcluster-v-ke-101    # 主集群中的 vCluster namespace
          port: 9090
```

> **版本说明**：新版直接路由到主集群 `prometheus-operated`，无需依赖 vCluster Syncer 的 fromHost mapService；旧版通过 Syncer 将主集群的 prometheus-operated 映射进 vCluster namespace，再从 vCluster namespace 访问。两种方式效果等同，新版更简洁。
