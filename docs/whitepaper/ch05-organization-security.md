...
本章介绍平台的多租户隔离架构、六层权限模型、认证审计机制和数据安全策略。
...
平台在 K8s 原生 RBAC 的基础上，扩展了六层 Scope（作用域）授权模型：

```
Platform（平台全局）       ← 最高权限，平台管理员
    ↓
CrossCluster（跨集群）     ← 跨集群路由，访问其他集群资源时触发
    ↓
Cluster（集群级）           ← 集群管理员
    ↓
...
> **源码实证**：`pkg/constants/scope.go` 定义了全部六种 Scope 类型：`platform`、`cross-cluster`、`cluster`、`workspace`、`nodegroup`、`namespace`。其中 `cross-cluster` 在请求目标集群与当前集群不同时由 `extractScopeFromRequestInfo` 提前识别并处理，其余五层由 `checkScopeChain` 按就近原则逐级检查。
...
>     if chain.NodeGroup != "" {
>         if result := a.checkAtScope(ctx, attrs,
>             constants.ScopeTypeNodegroup, chain.NodeGroup); result.Allowed {
>             result.Level = constants.ScopeTypeNodegroup
>             return result
>         }
>     }
>
...
>     if chain.Cluster != "" {
>         if result := a.checkAtScope(ctx, attrs,
>             constants.ScopeTypeCluster, chain.Cluster); result.Allowed {
>             result.Level = constants.ScopeTypeCluster
>             return result
>         }
>     }
>
...
>     if chain.Platform != "" {
>         if result := a.checkAtScope(ctx, attrs,
>             constants.ScopeTypePlatform, chain.Platform); result.Allowed {
>             result.Level = constants.ScopeTypePlatform
>             return result
>         }
>     }
>
...
六层 Scope 检查需要多次查询 ClusterRole/ClusterRoleBinding，性能开销不可忽视。平台通过 SimpleCache 实现授权缓存：
...
Token 使用 **RS256**（RSA + SHA-256）签名算法，确保不可篡改。Authorizer 通过验证 Token 签名获取用户身份，然后执行六层 Scope 授权检查。
...
| 六层授权 | CrossCluster→Namespace→Workspace→NodeGroup→Cluster→Platform | 精确到每个层级的权限控制 |
...
这套权限体系的核心价值在于：**让"谁能在哪里做什么"成为系统级保证，而非运维约定。** 当多个团队共享一个边缘平台时，权限的精确性直接决定了平台的可信度。六层 Scope 模型确保了从全局管理员到项目开发者，每个角色只能看到和操作属于自己的资源。
