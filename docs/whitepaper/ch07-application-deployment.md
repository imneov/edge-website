...
> **注**：`DeploymentHistory` 字段类型定义已完成，写入逻辑为规划中功能，当前运行时此字段始终为空。以下为字段结构说明，不代表当前运行时行为。
...
Controller 为 3 个 Topology 同时执行滚动更新。（注：变更历史记录为规划中功能，当前版本 `DeploymentHistory` 字段始终为空，`Reason: Upgrade, FromVersion: 1.0.0, ToVersion: 1.1.0` 为未来预期行为。）
